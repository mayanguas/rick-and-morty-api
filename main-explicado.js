...
const printCharacters = (list) => {
  ...
  // for (const item of list) { --> También podemos usar un for...of
  list.forEach((item) => {
    ...
    <div class="text-content img-content">
      ...
      <div class="text-position">
        ...
        <h4><span class="text-color">Especie:</span> ${item.specie}</h4>
        <h4><span class="text-color">Estado:</span> <span class="text-status">${item.status}</span></h4>
      </div>
...
nextBtn.addEventListener('click', () => {
  pageNum++; // También podemos poner: pageNum = pageNum + 1, es lo mismo
  getCharacters();
});
